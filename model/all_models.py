...
result_dir = time_filter_dir + '/fold_' + str(fold_num) + \
        '_4models_k_' + str(k) + '_lambda_' + str(lambd) + '_alpha_' + str(regl1jpp) + '_eta_' + str(eta)
...
(W, H, M) = JPPTopic(X, H1, topic_num, lambd, regl1jpp, epsilon, 200, True)
...
for current_time_step in range(start+1, finT+1):
    ...
    for fold_id in range(fold_num):
        ...
        P2, Q2, S2 = JPPCF_with_topic(Rt, Po2, Ct, k, eta, lambd, regl1jpp,  epsilon, maxiter, True)
        PredictR2 = ((1-eta)*np.dot(P2, Q2)) + (eta*Ct)
        NormPR2 = PredictR2 / PredictR2.max()

        logging.info('[ok]\n')

        logging.info('computing t-model NMF decomposition...')
        ...
        logging.info('computing fix_model NMF decomposition...')
    ...
    for recall_num in [3,5,10,20,50,100,150,200,250,300]:
        ...
        if not exist:
            result_file.write('t-model-nmf\tfix-model-nmf\tjppcf\tjppcf_with_topic\n')

        result_file.write(str(avg_tnmf_recall) + '\t' + str(avg_fnmf_recall) + \
                '\t' + str(avg_jppcf_recall) + '\t' + str(avg_jppcf_with_topic_recall) + '\n')
        result_file.close()
        ...
        result_file.write(str(avg_tnmf_ndcg) + '\t' + str(avg_fnmf_ndcg) + \
                '\t' + str(avg_jppcf_ndcg) + '\t' + str(avg_jppcf_with_topic_ndcg) + '\n')
        result_file.close()
        ...
        result_file.write(str(avg_tnmf_ap) + '\t' + str(avg_fnmf_ap) + \
                '\t' + str(avg_jppcf_ap) + '\t' + str(avg_jppcf_with_topic_ap) + '\n')
        result_file.close()
